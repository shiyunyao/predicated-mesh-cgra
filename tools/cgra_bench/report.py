...
def report(out: pathlib.Path, corpus: dict[str, Any], results: list[dict[str, Any]], family_manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    summary = {
        ...
        "backend_metrics": {
            ...
            "post_mapping_rejected": sum(stats.get("post_mapping_rejected", 0) for stats in backend_stats),
            "post_mapping_abort": sum(stats.get("post_mapping_abort", 0) for stats in backend_stats),
            "stage_rejected": sum(stats.get("stage_rejected", 0) for stats in backend_stats),
            ...
            "rf_port_match_calls": sum(stats.get("rf_port_match_calls", 0) for stats in backend_stats),
            "rf_port_match_failures": sum(stats.get("rf_port_match_failures", 0) for stats in backend_stats),
            "rf_read_port_early_rejects": sum(stats.get("rf_read_port_early_rejects", 0) for stats in backend_stats),
            "rf_write_port_early_rejects": sum(stats.get("rf_write_port_early_rejects", 0) for stats in backend_stats),
            "rf_write_source_early_rejects": sum(stats.get("rf_write_source_early_rejects", 0) for stats in backend_stats),
            "rf_port_events_committed": sum(stats.get("rf_port_events_committed", 0) for stats in backend_stats),
            "rf_port_rollback_count": sum(stats.get("rf_port_rollback_count", 0) for stats in backend_stats),
            "late_read_port_conflicts": sum(stats.get("late_read_port_conflicts", 0) for stats in backend_stats),
            "late_write_port_conflicts": sum(stats.get("late_write_port_conflicts", 0) for stats in backend_stats),
