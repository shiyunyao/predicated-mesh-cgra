#!/usr/bin/env python3
"""Conservative, frontend-independent feature inventory for LLVM assembly."""
...
OPCODE_RE = re.compile(r"^\s*(?:[%@][^ ]+\s*=\s*)?(?:tail\s+|musttail\s+|notail\s+)?([A-Za-z][A-Za-z0-9_.]*)\b")
...
def scan(path: pathlib.Path) -> dict:
    ...
    for line in text.splitlines():
        ...
        if not line[:1].isspace():
            continue
        # Switch case labels such as ``i32 8, label %case`` are indented in
        # LLVM assembly but are not instructions.
        ...
        if opcode == "phi":
            ...
            if "*" in line or re.search(r"\bptr\b", line):
                pointer_phis += 1
        if opcode == "br": branches += 1
        if opcode == "load": loads += 1
        if opcode == "store": stores += 1
        if opcode == "getelementptr": geps += 1
        if opcode in {"call", "invoke", "callbr"}: calls += 1
        if opcode in {"atomicrmw", "cmpxchg", "fence"}: atomics += 1
        if " volatile " in f" {line} ": volatile += 1
        if opcode == "select": selects += 1
        if opcode == "icmp":
            match = re.search(r"icmp\s+([a-z]+)", line)
            if match: predicates[match.group(1)] += 1
    return {
        ...
        "counts": {"basic_blocks": blocks, "phis": phis, "pointer_phis": pointer_phis, "branches": branches, "loads": loads, "stores": stores, "geps": geps, "calls": calls, "atomics_or_fences": atomics, "volatile": volatile, "selects": selects},
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"cgra-bench feature scan: {error}", file=sys.stderr)
        return 1
