...
BUILD_DIR = SIM_DIR / "power_activity_build"
...
def yosys_script(rtl_files: list[str], stat_path: pathlib.Path) -> str:
...
def run_activity(
    verilator: str, rtl_files: list[str], canonical: pathlib.Path, replay: pathlib.Path
) -> tuple[dict[str, object], list[str], list[str]]:
    build_log = SIM_DIR / "power_activity_build.log"
    run_log = SIM_DIR / "power_activity_run.log"
    replay_log = SIM_DIR / "power_activity_replay.log"
...
def run_power_probe(
    yosys: str, rtl_files: list[str], suffix: str
) -> tuple[dict[str, object], dict[str, object], list[str], pathlib.Path, pathlib.Path, pathlib.Path]:
    basename = f"asap7_power_small{suffix}"
    script_path = RAW_DIR / f"{basename}.ys"
    log_path = RAW_DIR / f"{basename}.log"
    stat_path = RAW_DIR / f"{basename}_stat.json"
    for path in (script_path, log_path, stat_path):
        path.unlink(missing_ok=True)
    script_path.write_text(yosys_script(rtl_files, stat_path), encoding="utf-8")
...
def render_report(record: dict[str, object]) -> str:
    ...
    liberty = record["liberty_power_data"]
    tool = record["tools"]
    capabilities = record["activity_capabilities"]
    memory_cells = record["mapping"]["unmodeled_memory_cells"]
    register_cells = record["mapping"]["register_cells"]
    ...
    lines = [
        ...
        "",
        "| Evidence | Result | Reproducibility |",
        "| --- | ---: | --- |",
        f"| Verilator RTL SAIF duration | {format_saif_duration(activity)} | two captures byte-identical |",
        f"| SAIF activity records | {activity['activity_records']} | canonical/replay SHA-256 match |",
        f"| SAIF total toggle count | {activity['total_toggle_count']} | existing 2x2 `cgra_top_tb` stimulus |",
        f"| ABC total power, raw | {power['total_raw']:g} | two probes identical |",
        f"| ABC static, raw | {power['static_raw']:g} | {power['frames']} frames, {power['prefix_frames']} prefix |",
        f"| ABC dynamic, raw | {power['dynamic_raw']:g} | internal {power['internal_raw']:g}; external {power['external_raw']:g} |",
        f"| ABC analyzed arcs | {power['arcs']} | wireload `{power['wireload']}` |",
        "",
        ...
        "",
        "- Verilator records actual RTL activity from the directed 2x2 configuration/run test at a 10 ns clock period.",
        "- ABC `power` uses its own internally generated switching-simulation frames after ASAP7 mapping. It does not consume the SAIF.",
        "- The values therefore must not be called workload-annotated power, energy per operation, calibrated power, or a small/default scaling comparison.",
        "",
        ...
        "## Library coverage",
        "",
        f"The pinned ASAP7 RVT/TT NLDM Liberty contains {liberty['internal_power_groups']} `internal_power` groups, {liberty['leakage_power_groups']} `leakage_power` groups, and {liberty['rise_power_tables']}/{liberty['fall_power_tables']} rise/fall power tables. The ABC probe covers only the mapped combinational snippet: it excludes {register_cells} mapped DFF cells, including their leakage, internal, and clock-pin power. It also leaves {memory_cells} `$mem_v2` cells unmodeled because no SRAM macro or SRAM power model is selected.",
        "",
...
def write_report(record: dict[str, object]) -> None:
    REPORT.write_text(render_report(record), encoding="utf-8")
...
def run() -> int:
    ...
    for path, description in (
        ...
        (TESTBENCH, "2x2 activity testbench"),
    ...
    before = input_snapshot(rtl_files)
    activity_path = RAW_DIR / "asap7_power_activity_small.saif"
    activity_replay_path = RAW_DIR / "asap7_power_activity_small_replay.saif"
    try:
        activity, activity_build_command, activity_run_command = run_activity(
            verilator, rtl_files, activity_path, activity_replay_path
        )
        ...
        replay_power, replay_resource, replay_command, replay_script, replay_log, replay_stat = (
            run_power_probe(yosys, rtl_files, "_replay")
        )
    except (OSError, RuntimeError, subprocess.SubprocessError) as error:
        write_blocked_report([str(error)])
        print(error, file=sys.stderr)
        return 1
    after = input_snapshot(rtl_files)
    if before != after:
        write_blocked_report(["A durable flow input changed during execution."])
        return 1
    if power != replay_power:
        write_blocked_report(["Independent ABC power probes produced different metrics."])
        return 1
    if sha256(stat_path) != sha256(replay_stat):
        write_blocked_report(["Independent ABC mapping stat JSON files are not identical."])
        return 1
    if resource != replay_resource:
        write_blocked_report(["Independent ABC mapping resource summaries differ."])
        return 1

    liberty = liberty_summary()
    manifest = json.loads(area.ASAP7_MANIFEST.read_text(encoding="utf-8"))
    metadata_path = RAW_DIR / "asap7_power_small.json"
    record = {
        ...
        "rtl_activity": {
            **activity,
            "source": "Verilator RTL SAIF from tb/cgra_top_tb.sv",
            "power_annotation_status": ANNOTATION_STATUS,
            "canonical": artifact(activity_path),
            "replay": artifact(activity_replay_path),
        ...
        "artifacts": {
            "activity_build_log": artifact(SIM_DIR / "power_activity_build.log"),
            "activity_run_log": artifact(SIM_DIR / "power_activity_run.log"),
            "activity_replay_log": artifact(SIM_DIR / "power_activity_replay.log"),
...
def main(argv: list[str]) -> int:
    ...
    self_test()
    return 0 if args.self_test else run()
