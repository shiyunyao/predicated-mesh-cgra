...
CGRA_BENCH_RF_PORT_AWARE ?= on
CGRA_BENCH_MAPPING_OBJECTIVE ?= find-any-feasible
...
help:
	...
	@echo "make cgra-bench-research-audit  Run the typed abstract mapping-lane audit"
	@echo "make cgra-bench-coverage-audit-local  Run the local enabled coverage lane without rewriting case metadata"
...
cgra-bench-smoke: cgra-bench-inventory
	...
	python3 tools/cgra_bench/run.py --corpus "$(CGRA_BENCH_CORPUS)" --cases benchmarks/cgra-bench/smoke_cases.v1.json --target target/cgra_v3.json --frontend-bin "$(CGRA_BENCH_FRONTEND_BIN)" --compile-kernel-bin "$(CGRA_BENCH_COMPILE_BIN)" --out "$(CGRA_BENCH_OUT)/smoke" --all --allow-subset --profile smoke; \
	python3 tools/cgra_bench/check_baseline.py --run "$(CGRA_BENCH_OUT)/smoke" --expectations benchmarks/cgra-bench/smoke_expectations.v1.json
...
cgra-bench-research-audit: cgra-bench-inventory
	@set -eu; \
	cmake -S compiler -B "$(CGRA_BENCH_FRONTEND_BUILD)" -DCGRA_BUILD_TESTS=OFF -DCGRA_WARNINGS_AS_ERRORS=ON $(LLVM_CMAKE_ARG); \
	cmake --build "$(CGRA_BENCH_FRONTEND_BUILD)" --target cgra-llvm-loop-lower cgrac-compile-kernel --parallel; \
	python3 tools/cgra_bench/run.py --corpus "$(CGRA_BENCH_CORPUS)" --cases "$(CGRA_BENCH_CASES)" --target "$(CGRA_BENCH_RESEARCH_TARGET)" --frontend-bin "$(CGRA_BENCH_FRONTEND_BIN)" --compile-kernel-bin "$(CGRA_BENCH_COMPILE_BIN)" --out "$(CGRA_BENCH_RESEARCH_OUT)" --all --profile research --lane mapping-research --source-abi m32

# This lane deliberately does not depend on inventory: inventory owns the
# upstream-derived manifests and must not erase checked-in adapter overrides.
cgra-bench-coverage-audit-local:
	@set -eu; \
	cmake -S compiler -B "$(CGRA_BENCH_FRONTEND_BUILD)" -DCGRA_BUILD_TESTS=OFF -DCGRA_WARNINGS_AS_ERRORS=ON $(LLVM_CMAKE_ARG); \
	cmake --build "$(CGRA_BENCH_FRONTEND_BUILD)" --target cgra-llvm-loop-lower cgrac-compile-kernel --parallel; \
	rotation_args=""; if test "$(CGRA_BENCH_ENABLE_SOFTWARE_ROTATION)" = 1; then rotation_args="--enable-software-rotation"; fi; \
	python3 tools/cgra_bench/run.py --corpus "$(CGRA_BENCH_CORPUS)" --cases "$(CGRA_BENCH_CASES)" --target "$(CGRA_BENCH_RESEARCH_TARGET)" --frontend-bin "$(CGRA_BENCH_FRONTEND_BIN)" --compile-kernel-bin "$(CGRA_BENCH_COMPILE_BIN)" --out "$(CGRA_BENCH_RESEARCH_OUT)" --all --profile research --lane mapping-research --source-abi m32 --mapping-objective "$(CGRA_BENCH_MAPPING_OBJECTIVE)" --enable-recurrence-ingress --rf-port-aware "$(CGRA_BENCH_RF_PORT_AWARE)" $$rotation_args
