...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout << "Array: " << target.array().rows << " x " << target.array().cols << "\n\n";
    ...
    std::cout << "Memory:\n  " << target.memory().model << "\n  " << target.memory().depth << ' '
              << target.memory().addressUnit << "s\n  " << target.memory().ports
              << " ports\n  load latency: " << target.memory().loadLatency << "\n\n";
    std::cout << "Networks:\n  registered: " << yesNo(target.dataNetwork().registeredLinks)
              << "\n  hop latency: " << target.dataNetwork().hopLatency
              << "\n  input buffering: " << yesNo(target.dataNetwork().inputBuffering) << "\n\n";
    ...
    std::cout << "Loop:\n  " << target.loopExecution().model
              << "\n  rotating registers: " << yesNo(target.loopExecution().rotatingRegisters)
              << '\n';
