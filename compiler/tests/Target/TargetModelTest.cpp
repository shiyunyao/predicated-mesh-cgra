...
void testCanonicalResources(const cgra::TargetModel& target) {
  ...
  check(target.dataRF().depth == 16 && target.dataRF().readPorts == 2 &&
        ...
        "DataRF resources");
  check(target.dataRF().writePortSources.at("W0") == std::vector<std::string>{"FU_DATA_RESULT"} &&
            target.dataRF().writePortSources.at("W1").size() == 9,
        "DataRF write-port source constraints");
  check(target.predicateRF().depth == 16 && target.predicateRF().readPorts == 2 &&
  ...
  check(target.dataNetwork().registeredLinks && target.dataNetwork().hopLatency == 1 &&
            !target.dataNetwork().inputBuffering && !target.dataNetwork().runtimeArbitration &&
            target.dataNetwork().compilerRouted && target.dataNetwork().separateResourceDomain &&
  ...
  check(
      target.loopExecution().supported && target.loopExecution().model == "finite_modulo_replay" &&
          !target.loopExecution().rotatingRegisters && !target.loopExecution().loopCounterOperand &&
          !target.loopExecution().sameAddressRfReadWriteRecurrence,
      "loop execution restrictions");
  check(target.controlLayout().rawWidth() == 126 && target.controlLayout().physicalWidth() == 128 &&
            target.controlLayout().chunks() == 4 && target.controlLayout().chunkBits() == 32 &&
  ...
  const std::map<std::string, std::map<std::string, unsigned>> expected = {
      ...
      {"route_predicate_source",
       {{"NONE", 0},
        ...
        {"CONST_TRUE", 8},
        {"CONST_FALSE", 9}}},
      {"lsu_op", {{"NONE", 0}, {"LOAD", 1}, {"STORE", 2}, {"RESERVED", 3}}},
  };

  for (const auto& [domain, entries] : expected) {
    for (const auto& [name, value] : entries) {
      check(target.encodingValue(domain, name) == value, "numeric encoding " + domain + "." + name);
...
void testKnownControl(const cgra::TargetModel& target) {
  ...
  const std::array<std::uint32_t, 4> expected = {0x50dd8402U, 0x6b378bd9U, 0x6ae716adU,
                                                 0x0310ae71U};
...
void testManifestRoundTrip(const cgra::TargetModel& target) {
  ...
  for (const auto& tile : manifest.at("program").at("tiles")) {
    for (const auto& entry : tile.at("control")) {
      ...
      check(roundTrip == original, "manifest control round-trip tile=(" +
                                       std::to_string(tile.at("row").get<unsigned>()) + "," +
                                       std::to_string(tile.at("col").get<unsigned>()) +
                                       ") pc=" + std::to_string(entry.at("pc").get<unsigned>()));
      ++count;
    }
  }
...
void testMalformedTargets(const Json& canonical) {
  expectRejected(
      canonical, [](Json& json) { json.erase("schema"); }, "schema");
  expectRejected(
      canonical, [](Json& json) { json["target_contract_version"] = 3; },
      "unsupported contract version");
  expectRejected(
      canonical, [](Json& json) { json["array"]["rows"] = 0; }, "array.rows");
  expectRejected(
      canonical, [](Json& json) { json["data_rf"]["read_ports"] = 0; }, "data_rf.read_ports");
  expectRejected(
      canonical, [](Json& json) { json["memory"]["ports"] = 3; },
      "enabled LSU count exceeds memory port count");
  expectRejected(
      canonical, [](Json& json) { json["encodings"]["op"]["PASS"] = 0; },
      "duplicate numeric value");
  expectRejected(
      canonical, [](Json& json) { json["encodings"]["op"]["ADD"] = 64; },
      "value does not fit field op");
  expectRejected(
      canonical, [](Json& json) { json["encodings"].erase("data_source"); },
      "missing encoding domain data_source");
  expectRejected(
      canonical,
      [](Json& json) { json["data_rf"]["write_ports_detail"]["W1"]["sources"][0] = "TYPO"; },
      "source TYPO is not in encoding domain data_source");
  expectRejected(
      canonical,
      [](Json& json) {
        json["predicate_rf"]["write_ports_detail"]["W1"]["sources"].push_back("CONST_FALSE");
      },
      "contains duplicate source CONST_FALSE");
  expectRejected(
      canonical, [](Json& json) { json["control_layout"]["fields"][1]["lsb"] = 0; },
      "overlaps another field");
  expectRejected(
      canonical, [](Json& json) { json["control_layout"]["fields"].back()["width"] = 5; },
      "extends beyond raw_width");
  expectRejected(
      canonical, [](Json& json) { json["control_layout"]["physical_width"] = 125; },
      "must be at least raw_width");
  expectRejected(
      canonical, [](Json& json) { json["control_layout"]["chunks"] = 3; },
      "exactly four 32-bit chunks");
  expectRejected(
      canonical, [](Json& json) { json["control_layout"]["padding"]["value"] = 1; },
      "only zero padding is supported");
  expectRejected(
      canonical, [](Json& json) { json["lsu"]["enabled_tiles"][0]["row"] = 4; },
      "outside the array");
  expectRejected(
      canonical, [](Json& json) { json["lsu"]["port_assignment"] = "arbitrary"; },
  ...
  expectRejected(
      canonical, [](Json& json) { json["control_schema"]["chunks"] = 3; }, "control_schema.chunks");
  expectRejected(
      canonical,
...
void testPaddingRejected(const cgra::TargetModel& target) {
  ...
  try {
    ...
    check(std::string(error.what()).find("padding bits must be zero") != std::string::npos,
          "padding diagnostic");
  }
}
